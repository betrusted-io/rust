...
impl File {
    pub fn open(path: &Path, opts: &OpenOptions) -> io::Result<File> {
        ...
        let (err, _) = request
            .lend_mut(
                services::pddb_server(),
                crate::os::xous::services::PddbLendMut::OpenKeyStd.into(),
            )
            .or_else(|_| {
    ...
    pub fn read(&self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        let (offset, valid) = crate::os::xous::ffi::lend_mut(
            services::pddb_server(),
            crate::os::xous::services::PddbLendMut::ReadKeyStd(self.fd).into(),
            &mut buffer.data,
            0,
    ...
    pub fn write(&self, buf: &[u8]) -> io::Result<usize> {
        ...
        let (offset, valid) = crate::os::xous::ffi::lend(
            services::pddb_server(),
            crate::os::xous::services::PddbLend::WriteKeyStd(self.fd).into(),
            &buffer.data,
            0,
            buf.len().min(buffer.data.len()),
        )
        .map_err(|_| {
    ...
    pub fn seek(&self, pos: SeekFrom) -> io::Result<u64> {
        let result = crate::os::xous::ffi::blocking_scalar(
            services::pddb_server(),
            crate::os::xous::services::PddbBlockingScalar::SeekKeyStd(self.fd, pos).into(),
        )
        .map_err(|_| {
...
impl Drop for File {
    fn drop(&mut self) {
        crate::os::xous::ffi::blocking_scalar(
            services::pddb_server(),
            crate::os::xous::services::PddbBlockingScalar::CloseKeyStd(self.fd).into(),
        )
        .unwrap();
    }
}
...
impl DirBuilder {
    ...
    pub fn mkdir(&self, p: &Path) -> io::Result<()> {
        ...
        request
            .lend_mut(
                services::pddb_server(),
                crate::os::xous::services::PddbLendMut::CreateDictStd.into(),
            )
            .or_else(|_| {
                Err(crate::io::Error::new(crate::io::ErrorKind::Other, "unable to query database"))
            })?;
...
pub fn readdir(p: &Path) -> io::Result<ReadDir> {
    ...
    request
        .lend_mut(
            services::pddb_server(),
            crate::os::xous::services::PddbLendMut::ListPathStd.into(),
        )
        .or_else(|_| {
            Err(crate::io::Error::new(crate::io::ErrorKind::Other, "unable to query database"))
        })?;
...
pub fn unlink(p: &Path) -> io::Result<()> {
    ...
    let (err, _) = request
        .lend_mut(
            services::pddb_server(),
            crate::os::xous::services::PddbLendMut::DeleteKeyStd.into(),
        )
        .or_else(|_| {
            Err(crate::io::Error::new(crate::io::ErrorKind::Other, "unable to query database"))
        })?;
...
pub fn rmdir(p: &Path) -> io::Result<()> {
    ...
    let (err, _) = request
        .lend_mut(
            services::pddb_server(),
            crate::os::xous::services::PddbLendMut::DeleteDictStd.into(),
        )
        .or_else(|_| {
            Err(crate::io::Error::new(crate::io::ErrorKind::Other, "unable to query database"))
        })?;
...
pub fn stat(p: &Path) -> io::Result<FileAttr> {
    ...
    request
        .lend_mut(
            services::pddb_server(),
            crate::os::xous::services::PddbLendMut::StatPathStd.into(),
        )
        .or_else(|_| {
            Err(crate::io::Error::new(crate::io::ErrorKind::Other, "unable to query database"))
        })?;

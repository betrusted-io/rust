...
impl UdpSocket {
        ...
        if let Ok(xous::Result::MemoryReturned(_, valid)) = response {
            ...
            if response[0] != 0 || valid.is_none() {
                ...
                if errcode == NetError::SocketInUse as u8 {
                    ...
                    return Err(io::Error::new_const(io::ErrorKind::Other, &"Library error"));
                } else {
    ...
    fn recv_inner(&self, buf: &mut [u8], do_peek: bool) -> io::Result<(usize, SocketAddr)> {
        ...
        let peek = if do_peek {
            Some(core::num::NonZeroUsize::new(1).unwrap())
        } else {
            None
        };
        if let Ok(xous::Result::MemoryReturned(_offset, _valid)) = xous::send_message(
            ...
            if receive_request.raw[0] != 0 { // error case
    ...
    pub fn send(&self, buf: &[u8]) -> io::Result<usize> {
        if let Some(addr) = self.remote.get() {
            ...
            Err(io::Error::new_const(io::ErrorKind::NotConnected, &"No remote specified"))
        }
    }
    ...
    pub fn send_to(&self, buf: &[u8], addr: &SocketAddr) -> io::Result<usize> {
        ...
        let write_timeout = if self.nonblocking.get() {
            ...
            // blocking
            if self.write_timeout.get() == 0 { // forever
                core::time::Duration::from_millis(u64::MAX)
            } else { // or this amount of time
            ...
            match response {
                Ok(xous::Result::MemoryReturned(_, valid)) => {
                    ...
                    if response[0] != 0 || valid.is_none() {
                        ...
                        if errcode == NetError::SocketInUse as u8 {
                            ...
                            return Err(io::Error::new_const(io::ErrorKind::InvalidInput, &"Socket not valid"));
                        } else if errcode == NetError::LibraryError as u8 {
                            return Err(io::Error::new_const(io::ErrorKind::Other, &"Library error"));
                        } else {
                            return Err(io::Error::new_const(io::ErrorKind::Other, &"Unable to connect"));
                        }
                    } else {
                        // no error
                ...
                Ok(xous::Result::RetryCall) | Err(xous::Error::ServerQueueFull) => {
                    if now.elapsed() >= write_timeout {
                        return Err(io::Error::new_const(io::ErrorKind::WouldBlock, &"Write timed out"));
                    } else {
                        // question: do we want to do something a bit more gentle than immediately retrying?
                        xous::yield_slice();
                    }
                }
                _ => return Err(io::Error::new_const(io::ErrorKind::Other, &"Library error")),
            }
        }
    }
...
impl Drop for UdpSocket {
    fn drop(&mut self) {
        if self.handle_count.fetch_sub(1, Ordering::Relaxed) == 1 { // only drop if we're the last clone
            match xous::send_message(
                services::network(),
                xous::Message::new_blocking_scalar(41 | ((self.fd as usize) << 16), // StdUdpClose
                0, 0, 0, 0)
            ) {

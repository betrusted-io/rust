...
pub fn lookup(query: &str, port: u16) -> Result<LookupHost, DnsError> {
    ...
    crate::os::xous::ffi::lend_mut(
        crate::os::xous::services::dns_server(),
        crate::os::xous::services::DnsLendMut::RawLookup.into(),
        &mut result.data,
        0,
        query.as_bytes().len(),
    )
    .unwrap();
    if result.data[0] != 0 {
        return Err(DnsError { code: result.data[1] });
    }
    assert_eq!(result.offset, 0);
    result.count = result.data[1] as usize;

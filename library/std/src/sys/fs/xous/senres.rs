        ...
        Ok(Message {
            ...
            data: unsafe {
                core::slice::from_raw_parts_mut(
                    crate::ptr::with_exposed_provenance_mut::<u8>(data),
                    len,
                )
            },
        })
    ...
    fn drop(&mut self) {
        if self.auto_return {
            ...
            return_memory_message(
                self.message_id,
                self.data.as_ptr().addr() as usize,
                self.data.len(),
            );
        }
    }
...
pub trait Senres {
    ...
    fn lend(&self, _connection: u32, _opcode: usize) -> Result<(), ()> {
        Ok(())
    }
    ...
    fn lend(&self, connection: u32, opcode: usize) -> Result<(), ()> {
        ...
        let a3 = opcode;
        let a4 = self.as_ptr().addr() as usize;
        let a5 = self.len();
...
pub trait SenresMut: Senres {
    ...
    #[cfg(target_os = "xous")]
    fn lend_mut(&mut self, connection: Connection, opcode: usize) -> Result<(usize, usize), ()> {
        crate::os::xous::ffi::lend_mut(connection, opcode, self.as_mut_slice(), 0, 0)
            .map_err(|_| ())
    }
}
